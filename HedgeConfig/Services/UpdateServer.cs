...
using System.Net.Http; // i know VS doesn't like this but it works only with it

namespace HedgeConfig.Services
{
    public static class UpdateServer
    {
        public static void CheckForUpdate(bool forcePrompt = false)
        {
            try
            {
                    ...
                    if (!string.IsNullOrWhiteSpace(releasesJson))
                    {
                        try
                        {
                            ...
                            foreach (var el in doc.RootElement.EnumerateArray())
                            {
                                ...
                                bool isPrerelease = el.TryGetProperty("prerelease", out var prereleaseProp) && prereleaseProp.GetBoolean();
                                bool isDraft = el.TryGetProperty("draft", out var draftProp) && draftProp.GetBoolean();
                            ...
                            // Get the latest version that's greater than current
                ...
                bool shouldPrompt = forcePrompt || (latestVersion != null && currentVersion != null && latestVersion > currentVersion);
